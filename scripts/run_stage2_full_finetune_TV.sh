...
STAGE1_CKPT_INPUT="${3:-${PROJECT_ROOT}/checkpoint/stage1_visual_change_pretrain_TV}"
OUTPUT_DIR="${4:-${PROJECT_ROOT}/checkpoint/stage2_train_final_val}"
GPU_IDS="${5:-0,1,2,3}"
...
RUN_NAME="${RUN_NAME:-stage2_train_final_val}"
...
STAGE1_CKPT_RESOLVED="$(resolve_model_path "${STAGE1_CKPT_INPUT}")"
FULL_MODEL_INIT_RESOLVED="$(resolve_model_path "${FULL_MODEL_INIT_PATH}")"

mkdir -p "${OUTPUT_DIR}" "${LOG_DIR}"

echo "======================================================"
echo "  Stage2 Full Finetune - TV"
...
echo "  FULL_LOAD    : ${FULL_MODEL_INIT_RESOLVED:-<none>}"
...
for required_path in "${TRAIN_FINAL}" "${VAL_FINAL}"; do
...
if [ -n "${FULL_MODEL_INIT_PATH}" ] && [ ! -d "${FULL_MODEL_INIT_RESOLVED}" ] && [ ! -f "${FULL_MODEL_INIT_RESOLVED}" ]; then
...
echo "======================================================"
echo "  Running Stage2-TV"
echo "  TRAIN_DATA : ${TRAIN_FINAL}"
echo "  VAL_DATA   : ${VAL_FINAL}"
echo "  OUTPUT_DIR : ${OUTPUT_DIR}"
...
cmd=(
    ...
    --data_path "${TRAIN_FINAL}"
    --dataset_type "change_detection"
    --val_data_path "${VAL_FINAL}"
...
if [ -n "${FULL_MODEL_INIT_PATH}" ]; then
    cmd+=(--full_model_init_path "${FULL_MODEL_INIT_RESOLVED}")
fi

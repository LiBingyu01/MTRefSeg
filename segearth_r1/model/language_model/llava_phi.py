...
from ..change_modules import BiTemporalFeatureFusion
...
class segearth_r1(PhiForCausalLM, LlavaMetaForCausalLM):
    ...
    def __init__(self, config, mask_decoder_cfg=None, add_cross_attn=True, cross_attn_index=None, use_seg_query=False):
        ...
        self.set_temporal_fusion_type(getattr(config, "temporal_fusion_type", "change_aware"))
    ...
    def reset_change_module_parameters(self):
        self.change_feature_fusion.reset_parameters()

    def set_temporal_fusion_type(self, fusion_type: str):
        self.config.temporal_fusion_type = fusion_type
        self.change_feature_fusion = BiTemporalFeatureFusion(
    ...
    def output_shape(self):
        ...
        out_feature_channels = self.backbone_feature_channels()
    ...
    def fuse_temporal_features(self, image_features_t1, image_features_t2=None):
        return self.change_feature_fusion(image_features_t1, image_features_t2)

    def build_decoder_features(self, images, images_t2=None):
        image_features = self.get_vision_tower_feature(images)
        if images_t2 is None:
            return image_features

        image_features_t2 = self.get_vision_tower_feature(images_t2)
        return self.fuse_temporal_features(image_features, image_features_t2)

    def build_seg_embedding_from_hidden(self, hidden_states, refer_embedding_indices, decoder_features):
        if refer_embedding_indices is None:
            return None

        SEG_embedding = self.get_SEG_embedding(hidden_states, refer_embedding_indices, return_all=True)
        origin_SEG_embedding = torch.cat(
            [self.origin_SEG_token_projector(kk.unsqueeze(0)[:, 0:1]) for kk in SEG_embedding]
        )
        ...
        for batch_idx, cur_SEG_embedding in enumerate(SEG_embedding):
            ...
            cur_SEG_embedding = self.d_layers(
                latents=cur_SEG_embedding.unsqueeze(1),
        ...
        return self.SEG_token_projector(SEG_embedding)

    def predict_masks_from_features(self, decoder_features, seg_query=None, SEG_embedding=None):
        mask_features, _, multi_scale_features = self.pixel_decoder.forward_features(decoder_features)
    ...
    def build_change_targets(self, masks):
        targets = []
        for mask in masks:
            targets.append(
                {
                    "labels": torch.tensor([1], device=mask.device, dtype=torch.int64),
                    "masks": mask,
                }
            )
        return targets

    def compute_mask_losses(self, mask_outputs, masks):
        mask_losses = self.criterion(mask_outputs, self.build_change_targets(masks))
        weight_dict = self.weight_dict

        loss_mask = 0.0
        loss_dice = 0.0
        loss_SEG_class = 0.0
        for key in list(mask_losses.keys()):
            if key not in weight_dict:
                mask_losses.pop(key)
                continue
            if mask_losses[key] is not None:
                mask_losses[key] *= weight_dict[key]
            if "_SEG" in key and mask_losses[key] is not None:
                loss_SEG_class += mask_losses[key]
            elif "_mask" in key:
                loss_mask += mask_losses[key]
            elif "_dice" in key:
                loss_dice += mask_losses[key]

        mask_loss = loss_mask + loss_dice + loss_SEG_class
        if isinstance(loss_mask, float):
            loss_mask = torch.tensor(loss_mask, device=mask_loss.device)
        if isinstance(loss_dice, float):
            loss_dice = torch.tensor(loss_dice, device=mask_loss.device)
        if isinstance(loss_SEG_class, float):
            loss_SEG_class = torch.tensor(loss_SEG_class, device=mask_loss.device)

        return mask_loss, loss_mask, loss_dice, loss_SEG_class

    def forward_visual_change(
        self,
        images,
        images_t2=None,
        masks=None,
        return_mask_outputs=False,
    ):
        decoder_features = self.build_decoder_features(images, images_t2)
        mask_outputs = self.predict_masks_from_features(
            decoder_features,
            seg_query=None,
            SEG_embedding=None,
        )

        if return_mask_outputs:
            return mask_outputs

        zero_device = images.device if images is not None else self.device
        zero_tensor = torch.tensor(0.0, device=zero_device)

        if masks is not None:
            mask_loss, loss_mask, loss_dice, loss_SEG_class = self.compute_mask_losses(mask_outputs, masks)
        else:
            mask_loss = zero_tensor
            loss_mask = zero_tensor
            loss_dice = zero_tensor
            loss_SEG_class = zero_tensor
        ...
        return CausalOutputWithMask(
            ...
            loss_llm=zero_tensor.detach(),
        )

    def eval_visual_change(self, images, images_t2=None):
        mask_outputs = self.forward_visual_change(
            images=images,
            images_t2=images_t2,
            masks=None,
            return_mask_outputs=True,
        )
    ...
    def concat_image_seg_embeds(self, input_id, img_feature, label, seg_query, seg_query_mask,
        ...
        # Support list of image features for multi-image input (e.g. bi-temporal change detection)
        if isinstance(img_feature, (list, tuple)):
            img_features_list = img_feature
        else:
            img_features_list = [img_feature]
        if seg_query is not None:
        ...
        cur_image_idx = 0  # Track which image feature to use for each <image> token
        cls_idx = 0
        for chunk in chunks:
            chunk_len = len(chunk)
            if chunk_len == 1 and chunk[0] == IMAGE_TOKEN_INDEX:
                # Select the appropriate image feature for this image token
                feat_idx = cur_image_idx if cur_image_idx < len(img_features_list) else -1
                cur_img_feat = img_features_list[feat_idx]
                cur_image_idx += 1
                cur_new_input_embeds.append(cur_img_feat)
                if seg_query is not None:
                    cur_new_seg_query_mask.append(torch.zeros(cur_img_feat.shape[0])) # [256,] all zeros
                if refer_embedding_indices is not None:
                    cur_refer_embedding_indices.append(
                        torch.full((cur_img_feat.shape[0],), 0, device=input_id.device,
                                   dtype=input_id.dtype)) 
                if label is not None:
                    cur_new_label.append(
                        torch.full((cur_img_feat.shape[0],), IGNORE_INDEX, device=label.device,
                                   dtype=label.dtype)
                    )
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # Encode T2 images if provided (for bi-temporal change detection)
        if images_t2 is not None:
            if type(images_t2) is list or images_t2.ndim == 5:
                concat_images_t2 = torch.cat([image for image in images_t2], dim=0)
                image_features_t2 = self.encode_images(concat_images_t2)
                split_sizes_t2 = [image.shape[0] for image in images_t2]
                image_features_t2 = torch.split(image_features_t2, split_sizes_t2, dim=0)
                image_features_t2 = [x.flatten(0, 1) for x in image_features_t2]
            else:
                image_features_t2 = self.encode_images(images_t2)
        else:
            image_features_t2 = None
        if use_seg_query:
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            # Build list of image features: [T1_feat] or [T1_feat, T2_feat] for change detection
            if image_features_t2 is not None:
                cur_img_features = [cur_image_feature, image_features_t2[batch_idx]]
            else:
                cur_img_features = cur_image_feature  # single tensor for backward compatibility
            ...
            if use_seg_query:
                cur_input_embeds, cur_label, cur_seg_query_mask, cur_refer_embedding_indices = self.concat_image_seg_embeds(
                    ...
                    img_feature=cur_img_features, # [256, 2048] or list thereof for multi-image
                ...
                cur_input_embeds, cur_label, cur_seg_query_mask, cur_refer_embedding_indices = self.concat_image_seg_embeds(
                    input_id=cur_input_ids, # [seq_len, ]
                    img_feature=cur_img_features, # [64, 2048] or list thereof for multi-image
                    label=cur_label, # [seq_len, ]
                    seg_query=None, # None
    ...
    def forward(
        ...
        images_t2: Optional[torch.FloatTensor] = None,
        ...
        if batch_dataset_type == "change_detection_stage1":
            return self.forward_visual_change(images=images, images_t2=images_t2, masks=masks)
        ...
        loss = None
        if batch_dataset_type == 'mm_conv' or batch_dataset_type == 'reason_seg' or batch_dataset_type == 'change_detection':
        ...
        if batch_dataset_type == 'refer_seg' or batch_dataset_type == 'reason_seg' or batch_dataset_type == 'change_detection':
            if self.use_seg_query:
                seg_query = self.seg_query_projector(self.get_seg_query(hidden_states, seg_query_mask))
            else:
                seg_query = None
            decoder_features = self.build_decoder_features(images, images_t2)
            SEG_embedding = self.build_seg_embedding_from_hidden(
                hidden_states,
    ...
    def eval_seg(
        ...
        if input_ids is None:
            return self.eval_visual_change(images=images, images_t2=images_t2)

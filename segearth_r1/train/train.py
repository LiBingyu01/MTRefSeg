...
from segearth_r1.train.change_dataset import ChangeDetectionDataset, ChangeDataCollector
from segearth_r1.eval_and_test.eval_dataset.change_val_dataset import ChangeValDataset, ChangeValDataCollector
from segearth_r1.train.llava_trainer import ChangeDetectionEvalCallback
...
def load_checkpoint_state_dict(checkpoint_path):
    if os.path.isdir(checkpoint_path):
        ...
        checkpoint_file = next((path for path in candidate_files if os.path.isfile(path)), None)
        if checkpoint_file is None:
            raise FileNotFoundError(f"No model weight file found under {checkpoint_path}")
    else:
...
def load_stage1_visual_weights(model, checkpoint_path):
    visual_prefixes = (
        "model.vision_tower.",
        "model.mm_projector.",
        "pixel_decoder.",
    ...
    if not filtered_state_dict:
        raise ValueError(f"No stage1 visual weights matched under {checkpoint_path}")

    load_msg = model.load_state_dict(filtered_state_dict, strict=False)
    print(f"[Stage2] Loaded {len(filtered_state_dict)} visual tensors from {checkpoint_path}")
    print(load_msg)


def load_full_model_weights(model, checkpoint_path):
    state_dict = load_checkpoint_state_dict(checkpoint_path)
    load_msg = model.load_state_dict(state_dict, strict=False)
    print(f"[Init] Loaded full model weights ({len(state_dict)} tensors) from {checkpoint_path}")
    print(load_msg)
...
class ModelArguments:
    ...
    mask_config: Optional[str] = field(default="./segearth_r1/mask_config/maskformer2_swin_base_384_bs16_50ep.yaml")
    stage1_pretrained_path: Optional[str] = field(default=None)
    full_model_init_path: Optional[str] = field(default=None)
    temporal_fusion_type: Optional[str] = field(default="change_aware")
...
class DataArguments:
    ...
    val_dataset_type: Optional[str] = field(default=None,
                                            metadata={"help": "Type of validation dataset (for cross-domain testing)."})
    max_eval_samples: Optional[int] = field(default=None,
                                            metadata={"help": "Optional cap on validation samples per epoch."})
...
def find_all_linear_names(model):
    ...
    for name, module in model.named_modules():
        if isinstance(module, cls):
            # Use the full module path so PEFT matches the intended leaf Linear
            # modules exactly, instead of accidentally matching parent containers
            # such as "model.layers.0" via a short numeric suffix.
...
def make_unify_datamodule(tokenizer, data_args, training_args):
    ...
    if data_args.dataset_type == 'change_detection':
        print("current train dataset is change_detection (bi-temporal)")
        Data = ChangeDetectionDataset(base_data_path=base_data_path, tokenizer=tokenizer, split='train')
    if data_args.dataset_type == 'change_detection_stage1':
        print("current train dataset is change_detection_stage1 (visual-only bi-temporal)")
        Data = ChangeDetectionDataset(
    ...
    ValData = None
    if data_args.val_data_path is not None and data_args.val_dataset_type is not None:
        val_base_path = data_args.val_data_path
        val_type = data_args.val_dataset_type
        print(f"Loading validation dataset: {val_type} from {val_base_path}")

        if val_type == 'RRSIS-D':
            ValData = RRSISDDataset(base_data_path=val_base_path, tokenizer=tokenizer)
        elif val_type == 'EarthReason':
            ValData = ReasonSegDataset(base_data_path=val_base_path, tokenizer=tokenizer)
        elif val_type == 'RefSegRS':
            ValData = RefSegRSDataset(base_data_path=val_base_path, tokenizer=tokenizer)
        elif val_type == 'MM_Conv':
            ValData = MM_Conv_Dataset(base_data_path=val_base_path, tokenizer=tokenizer, data_args=data_args)
        elif val_type == 'change_detection':
            ValData = ChangeDetectionDataset(base_data_path=val_base_path, tokenizer=tokenizer, split='val')
        elif val_type == 'change_detection_stage1':
            ValData = ChangeDetectionDataset(
                base_data_path=val_base_path,
                tokenizer=tokenizer,
                split='val',
                visual_only=True,
            )
        ...
        if ValData is not None:
            print(f'total validation dataset number is {len(ValData)}')

    if data_args.dataset_type == 'change_detection' or data_args.dataset_type == 'change_detection_stage1':
        data_collator = ChangeDataCollector(tokenizer=tokenizer)
    else:
        data_collator = DataCollector(tokenizer=tokenizer)
    return dict(train_dataset=Data, eval_dataset=ValData, data_collator=data_collator)

def train():
    global local_rank  
    parser = transformers.HfArgumentParser(
        (ModelArguments, DataArguments, TrainingArguments))
    model_args, data_args, training_args = parser.parse_args_into_dataclasses() # model_args, data_args, training_args
    ...
    if model_args.full_model_init_path is not None:
        load_full_model_weights(model, model_args.full_model_init_path)
    if model_args.stage1_pretrained_path is not None:
        load_stage1_visual_weights(model, model_args.stage1_pretrained_path)
    ...
    # 变化检测任务：每轮结束后在验证集上评测
    callbacks = []
    if data_args.dataset_type == 'change_detection' and data_args.val_data_path is not None:
        val_dataset = ChangeValDataset(
            base_data_path=data_args.val_data_path,
            tokenizer=tokenizer,
            split='val',
        )
        val_collator = ChangeValDataCollector(tokenizer=tokenizer)
        callbacks.append(ChangeDetectionEvalCallback(
        ...
        print(f'[Train] ChangeDetectionEvalCallback registered, val samples={len(val_dataset)}')

...
from data import HybridDataset, ValDataset, collate_fn
...
def parse_args():
    ...
    parser.add_argument("--change_refer_seg_data", default="ChangeRef_split|train", type=str)
...
def main():
    ...
    if args.eval_only:
        ...
        train_dataset = HybridDataset(
            ...
            * world_size,
            ...
            change_refer_seg_data=args.change_refer_seg_data,
        )
    if args.no_eval:
        val_dataset = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.no_eval:
            ...
            miou, oiou = validate(val_loader, model_engine, epoch, logger, args)
            current_is_best = miou > best_score
            is_best_tensor = torch.tensor([1 if current_is_best else 0], dtype=torch.int, device=model_engine.device)
        ...
        if args.no_eval or is_best:
            ...
            if args.local_rank == 0:
                torch.save(
                    ...
                    os.path.join(
                        args.log_dir,
                        "meta_log_miou{:.3f}_oiou{:.3f}.pth".format(
                            best_score, cur_oiou
                        ),
                    ),
                )
